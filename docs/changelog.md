...
- **Write verification for `type` and `select`** — after dispatching `input`/`change` the element is re-read synchronously and then polled (50 ms, default deadline 500 ms). The outcome is reported in new optional result fields `verified`, `verification` (`matched` | `transformed` | `reverted`), `requestedValue`, `previousValue` (`previousChecked` for `--toggle`), `options`, and `hint`. A write the app rolls back (`reverted`) fails with `Value reverted: …` (`Checked state reverted: …` for `--toggle`) plus a hint on stderr, and the failure object is printed to stdout (`type --json`; `select` always prints JSON) — `verification: "reverted"` is the stable signal for scripts; a value the app reformats (masks, formatters) is a success with `verification: "transformed"` and `verified: false`. The `reverted` baseline is taken after `focus()`/`--clear` have settled, so an app that accepts the clear (or reformats the value on focus) but rejects the real write is still reported as reverted. A value the browser sanitizes to empty (e.g. `abc` into `type=number`) fails with `Browser discarded the value: …` and the field is restored to what it held right before the write (after `--clear`, to the pre-clear value, with an `input` event so the app sees the restore); a value the browser clamps or sanitizes back onto the field's *current* value (e.g. `50` into a `type=range` already at its `max` of 10) fails with `Browser did not take the value: …` with no events dispatched — a clamp that lands on a different value (`50` into a range at 5 → `10`) is a `matched` success carrying the normalized `value` and the original `requestedValue`; a field the app empties after a non-empty write fails with `Value cleared: …`; an element that leaves the document (or is re-mounted) after focus/`--clear` is re-resolved by selector before the write, and one that disappears after the write (for `--toggle`, after the click) fails explicitly instead of being verified against a stale node. Custom elements that reflect `value` asynchronously are verified against the requested value. `--verify-timeout <ms>` (0–4000; 0 = single synchronous check) tunes the deadline for apps that apply values on a transition/async render. Detection is React-specific by construction (React restores controlled values synchronously inside `dispatchEvent`); frameworks that leave a rejected value in the DOM are reported as `matched`.
- `type` and `select` fail fast when the target is a `<select>` with no `<option>` matching the value, listing the available values in the `options` field and the hint (previously reported success with `value: ""`).
- Shared helpers in `src/commands/interact/shared.ts` (`buildSetValueScript()`, the `NATIVE_VALUE_WRITER_SNIPPET`/`VERIFY_POLL_SNIPPET`/`SETTLE_SNIPPET`/`SELECT_OPTIONS_SNIPPET` script fragments, the verify-timeout constants and option wiring, `parseInteractResult()`) and `WriteVerificationSchema` in `src/schemas/interact.ts`.
...
- `type` and `select` (value mode) refuse `<input type=checkbox|radio>` (hint: `select --toggle`), `<input type=file>`, and elements with no value setter such as contenteditable `<div>`s (previously both set an expando property — or the checkbox/radio `value` attribute — and reported success). `type`'s human output appends a note when the app transformed or the browser normalized the requested value.
- On failure, `type --json` and `select` print the failure result object (`error`, `hint`, and any `verification`/`options`/`previousValue` fields) to stdout before exiting 1; previously only the error message reached stderr.
